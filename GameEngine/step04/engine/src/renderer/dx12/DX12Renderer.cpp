...
bool DX12Renderer::Init(void* windowHandle) {
    ...
    // Create root signature (empty for this simple triangle)
    ...
    if (FAILED(D3D12SerializeRootSignature(&rootSignatureDesc, D3D_ROOT_SIGNATURE_VERSION_1, &signature, &error))) {
        if (error) {
            std::cerr << "[DX12] Root signature error: " << (char*)error->GetBufferPointer() << "\n";
        }
        return false;
    }
    ...
    if (FAILED(D3DCompile(vertexShaderSource, strlen(vertexShaderSource), nullptr, nullptr, nullptr, "main", "vs_5_0", 0, 0, &vertexShader, &shaderError))) {
        if (shaderError) {
            std::cerr << "[DX12] Vertex shader error: " << (char*)shaderError->GetBufferPointer() << "\n";
        }
        return false;
    }
    ...
    if (FAILED(D3DCompile(pixelShaderSource, strlen(pixelShaderSource), nullptr, nullptr, nullptr, "main", "ps_5_0", 0, 0, &pixelShader, &shaderError))) {
        if (shaderError) {
            std::cerr << "[DX12] Pixel shader error: " << (char*)shaderError->GetBufferPointer() << "\n";
        }
        return false;
    }
    ...
    // Create dynamic vertex buffer (large enough for typical meshes)
    const UINT maxVertexBufferSize = sizeof(Vertex) * 1024;  // Up to 1024 vertices
    ...
    // Initialize vertex buffer view (will be updated per mesh)
...
void DX12Renderer::DrawMesh(const Mesh* mesh) {
    // Upload this mesh's vertex data into the shared upload buffer
